...
const sketch = (p) => {
  ...
  p.draw = function () {
    ...
    for (let i = 0; i < BLOB_VERTICES; i++) {
      ...
      const vertexStep = i / BLOB_VERTICES * p.TWO_PI;

      // generate and scale noise to act as multiplier for base radius
      const trigs = cordic(n - vertexStep, true, false);
      const noise = p.map(simplex.noise2D(trigs[0], trigs[1]), -1, 1, 1 - NOISE_INFLUENCE, 1 + NOISE_INFLUENCE);
